...
var _ = Describe("CSI dynamic provisioning Test", func() {
	...
	It("should install a deployment using the above PVC", func() {
		...
		if envAirgapped != "" {
			deployment, err := utils.CreateDeployment(ctx, tc, testDeploymentName, "nginx-deployment-volume", airgappedImage, testDeletePVCName, "/data", testNameSpaceName)
			Expect(err).NotTo(HaveOccurred())
			Expect(deployment).NotTo(BeNil())
		} else {
			deployment, err := utils.CreateDeployment(ctx, tc, testDeploymentName, "nginx-deployment-volume", stagingImage, testDeletePVCName, "/data", testNameSpaceName)
			Expect(err).NotTo(HaveOccurred())
			Expect(deployment).NotTo(BeNil())
		}
